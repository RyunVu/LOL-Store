export default function ReportDeleteModal({ deleteModal, selectedCount, onConfirm, onClose }) {
  const { report, isMultiple } = deleteModal
      ...
      <div className="bg-gray-800 rounded-xl w-full max-w-md border border-gray-700 shadow-2xl">
        <div className="p-6">
          <h3 className="text-lg font-bold text-white mb-3">
            ⚠️ Delete Report{isMultiple ? 's' : ''}
          </h3>
          <div className="mb-6 space-y-3">
            <p className="text-gray-300">
              Are you sure you want to{' '}
              <span className="text-red-400 font-semibold">permanently delete</span>{' '}
              {isMultiple ? `${selectedCount} report(s)` : 'this report'}?
            </p>
            {!isMultiple && report && (
              <div className="bg-gray-700/50 border border-gray-600 rounded-lg p-3 space-y-1">
                <p className="text-white font-medium text-sm">{report.reporterName}</p>
                <p className="text-gray-400 text-sm line-clamp-2">{report.reason}</p>
              </div>
            )}
