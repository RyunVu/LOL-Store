...
> **Quick start**
>
> These are the commands I usually run right after cloning. If you just need to get things working quickly, execute the block below and then skip to “Run the backend API” / “Run the frontend”.
>
> ```powershell
> # restore & build
> dotnet restore
...
> ```
>
> You can return to the full guide below for more explanation and troubleshooting.
...
```
  ...
  > Consider adding a `global.json` at the repo root to pin the SDK if you work on multiple projects.
...
```
  ...
  The frontend uses Vite; you may want to pin the Node version in an `.nvmrc` file or the `package.json` `engines` field to avoid surprises when collaborating.
...
```json
"Jwt": {
	"Key": "LoLStoreSecretKey",           // **change this or move to secrets**
	"Issuer": "LoLStore",
	"Audience": "LoLStoreUsers"
}
```

> **Tip:** remove `LoLStoreSecretKey` from version control once you've set your own. You can store both the JWT key and connection string in a development-only `appsettings.Development.json` like this:
>
> ```json
> {
>   "ConnectionStrings": {
>     "DefaultConnection": "Server=(localdb)\MSSQLLocalDB;Database=LoLStore;Trusted_Connection=True;"
>   },
>   "Jwt": {
>     "Key": "YourStrongDevSecretHere",
>     "Issuer": "LoLStore",
>     "Audience": "LoLStoreUsers"
>   }
> }
> ```
>
> The `.gitignore` in this repo already excludes `appsettings.Development.json`, so it’s safe to keep your values there.
>
...
CORS settings are read from `AllowLocalHost` in `appsettings.json`. When running the frontend on `http://localhost:5173` (the default Vite port), make sure that origin is included there or the API will reject browser requests.
...
```powershell
cd frontend
npm install   # or `npm ci` if you want a clean, reproducible install
npm run dev
```
